...
function clock_info(dateformat, timeformat)
    ...
    return date..spacer..markup.fg.focus(time)
end

-- {{{1 Network activity
function net_info(interface, widget_color)
	...
	if net_in < 1073741824 then
		net_in  = string.format('%5.2fM', net_in / 1048576);
	else
		net_in  = string.format('%5.2fG', net_in / 1073741824);
	end

	if net_out < 1073741824 then
		net_out = string.format('%5.2fM',  net_out / 1048576);
	else
		net_out = string.format('%5.2fG', net_out / 1073741824);
	end
	
	return  markup.bold('Net:') .. spacer ..
			markup.fg.color(widget_color, net_in) .. spacer ..
	       '/' .. markup.fg.color(widget_color, net_out)
end
...
-- {{{1 Loadavg
function load_info(widget_color)
...
function volume_info(widget_color)
	...
	vol:close()
	return 'Vol: '..markup.fg.color(widget_color, volume)
end
...
function temperature_info(widget_color)
	...
	for line in temp:lines() do
		if line:match("temp4$") then
			temperature = line:match("(%d+%.%d)°F")
		end
	end
	temp:close()
	return 'Temp: '..markup.fg.color(widget_color, 'foo F')
end
